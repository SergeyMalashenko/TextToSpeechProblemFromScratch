...
from tts_dataset       import get_postnet_dataset, collate_fn_postnet
...
def cleanup_old_checkpoints(checkpoint_dir: str | Path, keep_last_n: int) -> None:
    ...
    checkpoints = sorted(
        checkpoint_dir.glob("checkpoint_postnet_*.pth.tar"),
...
def main() -> None:
    ...
    full_dataset = get_postnet_dataset()
    ...
    resume_path = hp_get("resume_postnet_checkpoint", None)
    ...
    for epoch in range(start_epoch, hp.epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if global_step % get_checkpoint_every() == 0:
                ckpt_path = checkpoint_dir / f"checkpoint_postnet_{global_step}.pth.tar"
                save_checkpoint(
                    model=model,
                    optimizer=optimizer,
                    scaler=scaler,
