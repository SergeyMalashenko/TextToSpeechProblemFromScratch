...
HIDDEN = hp_get("postnet_hidden_dim", 512)
KERNEL = hp_get("postnet_kernel_size", 5)
N_CONVS = hp_get("postnet_n_convolutions", 5)
DROPOUT = hp_get("postnet_dropout", 0.5)
...
class ModelPostNet(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        layers.append(ConvBlock(N_MELS, HIDDEN, KERNEL, activation=True))
...
def run_model_postnet_smoke_test() -> None:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = ModelPostNet().to(device)
